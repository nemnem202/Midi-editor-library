...
export class TransportController {
  ...
  public loadNewMidi() {
    ...
    this.audio.onLoopEnd = () => {
      ...
      this.store.dispatch({
        ...
        loop: {
          ...state.config.loop,
          currentRepeatIndex: state.config.loop.currentRepeatIndex + 1,
        },
      });
      if (state.config.loop.currentRepeatIndex >= state.config.repeats) this._loopJumping = false;
      this.audio.setPlaybackRate(
        state.config.bpm + state.config.bpmPractice * state.config.loop.currentRepeatIndex
      );
      this.transposeAllChannels(
        state.config.transpositionPractice * state.config.loop.currentRepeatIndex
      );
  ...
  private async play() {
    ...
    if (!state) return;
    this.audio.setPlaybackRate(
      state.config.bpm + state.config.bpmPractice * (state.config.loop?.currentRepeatIndex ?? 0)
    );
  ...
  resume() {
    ...
    this.store.midiState?.config.loop &&
      this.store.dispatch({
        type: Action.SET_LOOP,
        loop: { ...this.store.midiState.config.loop, currentRepeatIndex: 0 },
      });
