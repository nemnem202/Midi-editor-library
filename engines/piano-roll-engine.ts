...
import NotesRenderer, {
	EditorNotesRenderer,
	PlayerNotesRenderer,
} from "../renderers/notes-renderer";
import PianoKeyboardRenderer, {
...
import PlayheadRenderer, {
	EditorPlayheadRenderer,
	PlayerTacklistRenderer,
} from "../renderers/playhead-renderer";
import ViewportRenderer, {
	EditorViewportRenderer,
	PlayerViewportRenderer,
} from "../renderers/viewport-renderer";
...
export abstract class PianoRollEngine {
	...
	public async init() {
		try {
			...
			SoundEngine.init(this.state, (tick) => this.onSoundEngineTickUpdate(tick))
				.then(() => {
					this.soundEngine = SoundEngine.get();
					this.soundEngine.updateMidiEvents();
				})
				.catch((err) => console.error("SoundEngine failed to init:", err));
	...
	protected processActions() {
		...
		if ([...actions].some((a) => a === Action.SET_TRANSPORT_START)) {
			const { start } = this.state.transport;
			this.soundEngine.setStartingTick(start);
			this.playheadRenderer.drawTracklist();
		}
		if ([...actions].some((a) => a === Action.TOGGLE_PLAY)) {
	...
	public destroy(): void {
		...
		logger.info("Engine destroyed safely");
	}
}
		...
		this.pointerHandler = new PointerActionHandler(
...

			{
				default: {
					...
					onAltDrag: {
						onStart: () =>
							this.cursorRenderer.setCursor("grabbing").lock("drag"),
