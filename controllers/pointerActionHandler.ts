...
import type { FederatedPointerEvent, FederatedWheelEvent, Application } from "pixi.js";
...
export class PointerActionHandler {
  ...
  private resolveContext(e: FederatedPointerEvent | FederatedWheelEvent): PointerActionMap {
    ...
    if (this.activeContext.onLongPress) {
      ...
      this.longPressTimer = setTimeout(() => {
        if (this.isPointerDown && !this.isDragging && this.longPressEvent) {
          ...
          this.activeContext!.onLongPress!(this.makeClickPayload(this.longPressEvent));
        }
      }, this.longPressDelay);
    }
  };
  ...
  private onPointerMove = (e: FederatedPointerEvent): void => {
    ...
    if (!this.isDragging) {
      ...
      if (this.pendingDownEvent && this.activeDragAction?.onStart) {
        this.activeDragAction.onStart(this.makeDragPayload(this.pendingDownEvent));
      }
  ...
  private onPointerUp = (e: FederatedPointerEvent): void => {
    ...
    const now = Date.now();
    if (now - this.lastClickTime < this.doubleClickDelay && !this.isDragging) {
      this.activeContext?.onDoubleClick?.(this.makeClickPayload(e));
  ...
  private onWheel = (e: FederatedWheelEvent) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      return isUp ? ctx.onCtrlWheelUp?.(payload) : ctx.onCtrlWheelDown?.(payload);
    }
    if (e.shiftKey) {
      return isUp ? ctx.onShiftWheelUp?.(payload) : ctx.onShiftWheelDown?.(payload);
    }
    if (e.altKey) {
