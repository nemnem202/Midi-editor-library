...
export default class SelectionRenderer extends Renderer<SelectionRendererDeps> {
  ...
  public handleDragEnd(e: FederatedPointerEvent, origin: PointerActionEvent["dragOrigin"]) {
  ...
  public handleDrag(e: FederatedPointerEvent, origin: PointerActionEvent["dragOrigin"]) {
  ...
  public unselectAll() {
    if (this.state.tracks[this.state.currentTrackId].data.selectedNotes.indexOf(1) === -1) return;
    this.dispatch({
  ...
  private extractSquareArea(e: FederatedPointerEvent, origin: PointerActionEvent["dragOrigin"]) {
  ...
  private selectNotes(square: { width: number; height: number; x: number; y: number }) {
    ...
    for (let i = 0; i < data.noteCount; i++) {
      ...
      const noteEnd = noteStart + data.durations[i];
      const noteTop = (128 - data.pitches[i]) * noteHeight;
